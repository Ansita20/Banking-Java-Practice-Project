...
const AccountPage = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    if (!profileData.adhaar || !profileData.pan || !profileData.mobile) {
    ...
    try {
      const resp = await axios.put(`${BASE_URL}/api/v1/user/updateprofile/${userDetails.userId}`, profileData);
  ...
  // If no account, show account opening request
  if (!accountNo) {
    return (
      <div className="min-h-screen bg-gray-100">
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-md">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Welcome to Online Banking</h2>
            <p className="text-gray-600 mb-6">You don't have an account yet. Apply for one now!</p>
            <button
                ...
                >
                  ...
                  {beneficiaries.map((b) => (
                    <option key={b.beneficiaryid} value={b.beneaccountno}>
                      {b.beneficiaryname} - {b.beneaccountno}
                    </option>
                  ))}
                </select>
              </div>
